...
with DAG(
    ...
    def replace_null_values():
        ...
        df = df.fillna('-')
        df.to_csv('/opt/airflow/data/step1.csv', index=False)

    def sort_by_date():
        df = pd.read_csv('/opt/airflow/data/step1.csv')
        df['created_date'] = pd.to_datetime(df['created_date'], errors='coerce')
        df = df.sort_values(by='created_date')
        df.to_csv('/opt/airflow/data/step2.csv', index=False)

    def clean_content():
        df = pd.read_csv('/opt/airflow/data/step2.csv')
    ...
    with TaskGroup(group_id='task_group_process_data') as process_data:
        t1 = PythonOperator(task_id='replace_null', python_callable=replace_null_values)
        t2 = PythonOperator(task_id='sort_by_date', python_callable=sort_by_date)
        t3 = PythonOperator(task_id='clean_content', python_callable=clean_content)
        t1 >> t2 >> t3
